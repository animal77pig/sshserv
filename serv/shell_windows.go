...
func handleShell(req *ssh.Request, channel ssh.Channel, pty *winpty.WinPTY, connection *Connection) bool {
	// Teardown session
	var once sync.Once
	close := func() {
		channel.Close()
	...
	go func() {
		connection.Copy(channel, pty.StdOut)
		once.Do(close)
	}()
...
func handlePtrReq(req *ssh.Request, wd string, perms []string) *winpty.WinPTY {
	...
	if err != nil {
		hh.Error("Failed to start command: %s, shell: %s", err.Error(), defaultShell)
		return nil
	}
	...
	for req := range in {
		...
		case "shell":
			if pty == nil {
				...
				ok = handleShell(req, channel, pty, connection)
			}
		case "window-change":
			if pty == nil {
				logger.Warn(logShell, "pty not open yet!")
