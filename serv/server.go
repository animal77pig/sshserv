...
func (c *Configuration) AcceptInboundConnection(conn net.Conn, config *ssh.ServerConfig) {
	...
	if err != nil {
		hh.Warn("failed to accept an incoming connection: %v", err)
		return
	}
	defer sconn.Close()

	hh.Info("accepted inbound connection, ip: %v", conn.RemoteAddr().String())

	var user dataprovider.User

	err = json.Unmarshal([]byte(sconn.Permissions.Extensions["user"]), &user)
	if err != nil {
		hh.Warn("Unable to deserialize user info, cannot serv connection: %v", err)
		return
	}
	...
	hh.Info("   --client version: %s, addr: %v\n", sconn.ClientVersion(), sconn.RemoteAddr())
	...
	for loop {
		...
		case r := <-reqs:
			...
			case "tcpip-forward":
				...
				if c.FullFunc && connection.User.HasPerm(dataprovider.PermTCPForward) {
					...
					ok, payload, _ = forwardHandler.handlePortforward(sconn, r, &connection)
				}
				if ok {
		...
		case <-time.After(time.Second * 5):
			ret := time.Now().Sub(connection.lastActivity)
			//hh.Info("time idle of %s is %.1f", remoteAddr, ret.Seconds())
			if int(ret.Seconds()) > c.IdleTimeout*60 {
				hh.Warn("session timeout, close session")
				loop = false
				break
			}
		}
	...
	if forwardHandler != nil {
		for ln_addr, ln := range forwardHandler.forwards {
			ln.Close()
			hh.Warn("   R ln_addr [%s] closed", ln_addr)
		}
	}
	removeConnection(connectionID)
	hh.Info("   ---------connection %s done", remoteAddr)
}
...
func (c *Configuration) iterChans(newChannel ssh.NewChannel, sconn *ssh.ServerConn, connection *Connection) bool {
	...
	hh.Info("  --- newChannel.ChannelType(): [%s]", chType)
	if chType == "direct-tcpip" {
		if c.FullFunc && connection.User.HasPerm(dataprovider.PermTCPForward) {
			go HandleDirectTCPIP(sconn, newChannel, connection)
			return true
		} else {
			hh.Warn("Denied -L port-forwarding of user %s", connection.User.Username)
			return false
		}
	}

	if chType != "session" {
		hh.Error("received an unknown channel type: %v, reject", chType)
	...
	if err != nil {
		hh.Warn("could not accept a channel: %v", err)
		return false
	}
...
/*
*
实现一个Copy函数， 以便更新 lastActivity 时间， 用于判断连接的超时
*/
func (c *Connection) Copy(dst io.Writer, src io.Reader) (written int64, err error) {
