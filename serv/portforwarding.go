...
func (h *ForwardedTCPHandler) handlePortforward(conn *ssh.ServerConn, req *ssh.Request, connection *Connection) (bool, []byte, string) {
	h.Lock()
	if h.forwards == nil {
	...
	case "tcpip-forward":
		...
		go func() {
			...
			for {
				c, err := ln.Accept()
				if err != nil {
					hh.Error("R accept failed %v", err)
					break
				}
...
func HandleDirectTCPIP(conn *ssh.ServerConn, newChan ssh.NewChannel, connection *Connection) {
