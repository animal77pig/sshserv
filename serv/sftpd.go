// Package serv implements the SSH File Transfer Protocol as described in https://tools.ietf.org/html/draft-ietf-secsh-filexfer-02.
...
//					hh.Debug("idle time: %v setted to transfer idle time: %v connection id: %v",
...
//			err := c.sshConn.Close()
//			hh.Warn("idle connection closed id: %v, err: %v", c.ID, err)
//		}
//	}
//	hh.Debug("check idle connections ended")
//}
...
func addConnection(id string, conn *Connection) {
	...
	hh.Debug("connection added, num open connections: %v", len(openConnections))
}
...
func removeConnection(id string) {
	...
	hh.Debug("connection removed, num open connections: %v", len(openConnections))
}
