...
func Connect(remote string, port int, user, pass_or_keyfile string) (*Cli, error) {

	auths := []ssh.AuthMethod{ssh.Password(pass_or_keyfile)}
...
func connectRemote(connstr string, key_file string, user string) (*Cli, string, error) {
	...
	co, err := Connect(s1[0], port, user, key_file)
	if err != nil {
		return nil, "", err
	}
...
*/
func fromRemote(path string, toFunc toFunc, dst interface{}, dstPath string, key_file string, user string) error {
	srcCli, srcPath, err := connectRemote(path, key_file, user) // connect src sftp
	if err != nil {
...
sftpcli -u root 192.168.2.10:22@/mnt/adas-rt/config/engine.json d:/tmp/
*/
func FileTran(src, dst, keyfile string, user string) error {
	var err error
	if strings.Index(src, "@") > 0 && strings.Index(dst, "@") > 0 {
		// remote to remote
		dstCli, dstPath, err := connectRemote(dst, keyfile, user) // connect src sftp
		if err != nil {
			fmt.Printf("failed connect to:" + dst)
			return err
		}
		defer dstCli.Close()
		err = fromRemote(src, toRemote, dstCli, dstPath, keyfile, user)
	} else if strings.Index(src, "@") > 0 {
		// remote to local
		err = fromRemote(src, toLocal, nil, dst, keyfile, user)
...
func main() {
	...
	err := FileTran(src, dst, *key_file, *user)
	if err != nil {
		os.Exit(1)
	}
