...
func runLsStatt_1(dirent os.FileInfo, statt *SftpFileAttr) string {
	...
	uid := statt.Uid
	gid := statt.Gid
	username := statt.Uname
	if username == "" {
		username = fmt.Sprintf("%d", uid)
	}
	groupname := statt.Gname
	if groupname == "" {
		fmt.Sprintf("%d", gid)
	}
	...
	return fmt.Sprintf("%s %4d %-8s %-8s %8d %s %2d %5s %s",
		typeword, numLinks, username, groupname, dirent.Size(), monthStr, day, yearOrTime, dirent.Name())
}
...
func runLs(dirname string, dirent os.FileInfo) string {
	...
	} else if statt1, ok := dsys.(*SftpFileAttr); ok {
		return runLsStatt_1(dirent, statt1)
	}
